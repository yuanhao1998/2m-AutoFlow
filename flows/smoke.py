...
class Home(State):
    ...
    def handle(self, ctx):
        self.log.info("在 HOME，点击打开面板")
        if not ctx.click(Target.image(Anchor(ref=img["home_open_panel"]))):
            self.log.warning("未匹配到打开面板按钮，本轮将重试")
        ctx.wait(2)
        return Goto(Panel)
...
class Popup(State):
    ...
    def handle(self, ctx):
        self.log.info("检测到弹框 → 关闭")
        if not ctx.click(Target.image(Anchor(ref=img["popup_close"]))):
            self.log.warning("未匹配到关闭弹框按钮，本轮将重试")
        ctx.wait(1)
        return Back()


def build_registry() -> StateRegistry:
    reg = StateRegistry(flow_name="smoke")
